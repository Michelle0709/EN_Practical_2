    ...
    for (int a = 1; a < argc; a++)
    {
        ...
        else if (strcmp(argv[a], "-t") == 0)
        {
            ...
            for (int b = 0; b < argLength; b++){
                inputText[b] = argv[a+1][b];
            }
            inputText[argLength] = '\0';
//            plaintext = inputText;
            plaintextLength = argLength;
            if (cbc){
                if (encrypt){
                    cbcStruct.plaintext = inputText;
                    cbcStruct.pSize = plaintextLength;
                    cbcStruct.cSize = (cbcStruct.pSize / 16 + 1) * 16;
                } else {
                    cbcStruct.ciphertext = inputText;
                ...
                if (encrypt){
                    ...
                    cfbStruct.plaintext = inputText;
                } else {
                    cfbStruct.pSize = plaintextLength;
                    cfbStruct.ciphertext = inputText;
                }
            }
            a++;
        }
...
//            unsigned char iv[argLength + 1];
        ...
        else if (strcmp(argv[a], "-streamlen") == 0)
        {
            ...
            if (cfb) {
                ...
                printf("Streamlen is not allowed for CBC operation. Please try again.\n");
                return 0;
            }
            a++;
        }
    ...
    if (cbc) {
        if (encrypt){
            ...
            if (fileIn){
                ...
                printArr(cbcStruct.ciphertext, cbcStruct.cSize, 'x');
            }
...
//            printArr(cfbStruct.plaintext, cfbStruct.pSize, 'c');
        ...
        if (encrypt){
            ...
            test_functionality(inputText, keyLength, key);
//            pad_and_encrypt(inputText, newArray, plaintextLength, keyLength, key);
        } else {
            general_decrypt(inputText, plaintextLength, keyLength, key);
        }
